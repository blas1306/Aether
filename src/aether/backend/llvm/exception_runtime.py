    ...
    def _create() -> str:
        ...
        return "\n".join(
            [
                ...
                "  br i1 %descriptor_ok, label %check_fault, label %panic",
                "check_fault:",
                "  br i1 %forced_fault, label %drop_and_panic, label %allocate",
                "allocate:",
    ...
    def _root_terminate(strategy: ExceptionLoweringStrategy) -> str:
        ...
        return "\n".join(
            [
                ...
                "  %forced_reporting_failure = or i1 %message_fault, %report_fault",
                "  br i1 %forced_reporting_failure, label %reporting_failure, label %message",
                "message:",
                "  %descriptor_ptr = getelementptr %AetherExceptionEventV1, ptr %event, i32 0, i32 3",
                "  %descriptor = load ptr, ptr %descriptor_ptr",

    ...
    def _convert_pure_instruction(
        ...
        if isinstance(instruction, IRInterfaceCall):
            return SSAInterfaceCall(
                self._resolve_value(instruction.receiver, state.value_map),
                tuple(
                    self._resolve_value(argument, state.value_map)
                    for argument in instruction.arguments
                ),
        ...
        if isinstance(instruction, IRListIndexOf):
            ...
            list_value = self._resolve_value(instruction.list_value, state.value_map)
            value = self._resolve_value(instruction.value, state.value_map)
            return SSAListIndexOf(result, list_value, value)

        if isinstance(instruction, IRListClear):
            list_value = self._resolve_value(instruction.list_value, state.value_map)
            return SSAListClear(list_value)
        ...
        if isinstance(instruction, IRListInsert):
            ...
            value = self._resolve_value(instruction.value, state.value_map)
            return SSAListInsert(list_value, index, value)
        ...
        if isinstance(instruction, IRListRemoveAt):
            ...
            list_value = self._resolve_value(instruction.list_value, state.value_map)
            index = self._resolve_value(instruction.index, state.value_map)
            return SSAListRemoveAt(result, list_value, index)
        ...
        if isinstance(instruction, IRVectorMatrixMul):
            ...
            vector = self._resolve_value(instruction.vector, state.value_map)
            matrix = self._resolve_value(instruction.matrix, state.value_map)
            return SSAVectorMatrixMul(result, vector, matrix, instruction.rows, instruction.cols)

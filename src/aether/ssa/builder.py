...
class SSABuilder:
    ...
    def _build_function(self, function: IRFunction) -> SSAFunction:
        ...
        if self._is_linear_function(function):
            ...
            instructions = self._build_block_instructions(block, state)
    ...
    def _build_simple_if_else_blocks(
        ...
        entry_instructions = self._build_block_instructions(
            simple_if_else.entry,
            entry_state,
        )

        then_state = entry_state.copy()
        then_instructions = self._build_block_instructions(
            simple_if_else.then_block,
            then_state,
        )

        else_state = entry_state.copy()
        else_instructions = self._build_block_instructions(
            simple_if_else.else_block,
            else_state,
        )

        blocks = [
            SSABasicBlock(simple_if_else.entry.name, entry_instructions),
            SSABasicBlock(simple_if_else.then_block.name, then_instructions),
            SSABasicBlock(simple_if_else.else_block.name, else_instructions),
        ]

        if simple_if_else.merge_block is None:
            return blocks

        merge_state = _BuildState(
            dict(entry_state.value_map),
            {},
        )
        merge_instructions, merge_slots, incomplete_slots = self._merge_slot_states(
        ...
        merge_state.slot_values = merge_slots
        merge_state.incomplete_slots = incomplete_slots
        merge_instructions.extend(
            self._build_block_instructions(simple_if_else.merge_block, merge_state)
        )
        blocks.append(
            SSABasicBlock(simple_if_else.merge_block.name, merge_instructions)
        )
    ...
    def _build_block_instructions(
        ...
        for instruction in block.instructions:
            ...
            if isinstance(instruction, IRJump):
                instructions.append(SSAJump(instruction.target))
                continue
    ...
    def _merge_slot_states(
    ...
    ) -> tuple[list[SSAInstruction], dict[str, SSAValue], set[str]]:
        ...
        for slot_name in sorted(slot_names):
            ...
            instructions.append(
                SSAPhi(result, ((then_name, then_value), (else_name, else_value)))
            )
            merged_slots[slot_name] = result

        return instructions, merged_slots, incomplete_slots

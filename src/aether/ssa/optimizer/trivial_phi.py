...
class TrivialPhiEliminator:
    ...
    def _rewrite_instruction(
        ...
        if isinstance(instruction, SSACallIndirect):
            callee, callee_rewritten = self._rewrite_value(
                instruction.callee, replacements
            )
            arguments = []
            rewritten_uses = int(callee_rewritten)
            for argument in instruction.arguments:
                rewritten_argument, rewritten = self._rewrite_value(
            ...
            if rewritten_uses == 0:
                return instruction, 0
            return (
                SSACallIndirect(callee, tuple(arguments), instruction.result),
                rewritten_uses,
            )
        ...
        if isinstance(instruction, SSAListSlice):
            ...
            if not list_rewritten and not start_rewritten and not end_rewritten:
                return instruction, 0
            return (
                SSAListSlice(instruction.result, list_value, start, end),
                int(list_rewritten) + int(start_rewritten) + int(end_rewritten),
            )
        ...
        if isinstance(instruction, SSAListInsert):
            ...
            index, index_rewritten = self._rewrite_value(instruction.index, replacements)
            value, value_rewritten = self._rewrite_value(instruction.value, replacements)
            count = int(list_rewritten) + int(index_rewritten) + int(value_rewritten)
            return (SSAListInsert(list_value, index, value), count) if count else (instruction, 0)

        if isinstance(instruction, SSAListPop):
            list_value, rewritten = self._rewrite_value(instruction.list_value, replacements)
            return (SSAListPop(instruction.result, list_value), 1) if rewritten else (instruction, 0)

        if isinstance(instruction, SSAListRemoveAt):
            list_value, list_rewritten = self._rewrite_value(instruction.list_value, replacements)
            index, index_rewritten = self._rewrite_value(instruction.index, replacements)
            count = int(list_rewritten) + int(index_rewritten)
            return (SSAListRemoveAt(instruction.result, list_value, index), count) if count else (instruction, 0)

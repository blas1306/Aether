        ...
        if lv.is_some() || rv.is_some() || lm.is_some() || rm.is_some() {
            ...
            if op == AstBinaryOp::Multiply {
                ...
                if left_math == right_math {
                    return Err(error(
                        "E0342",
                        "algebraic multiplication pairing unsupported; expected scaling, Row × Column, Column × Row, Matrix × Column or Row × Matrix",
                    ));
                }
...
fn vector_product_recipe(
    ...
    if !missing.is_empty() {
        return Err(format!(
            "algebraic Vector multiplication missing capabilities {}",
            missing.join(" + ")
        ));
    }
    let op = |behavior| {
        ...
        for inner in [true, false] {
            ...
            for case in 0..10 {
                ...
                match case {
                    ...
                    5..=9 => match product {
                        VectorProduct::MatrixVector { .. } => unreachable!(),
                        VectorProduct::Inner {
                            shape_check,
                            accumulate_op,
                            zero,

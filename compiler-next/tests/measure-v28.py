#!/usr/bin/env python3
"""V28 warm-cache snapshots using the eight V17 core phase boundaries.
...
Optional baseline binary must be built separately from the unmodified V27 tree.
"""
import argparse
import json
...
FIXTURES = (
    "v27_vector_contiguous.ae", "v28_vector_contiguous.ae",
    "v28_vector_strided.ae", "v28_matrix_contiguous.ae",
    "v28_matrix_transposed.ae",
)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="aether-v28-timing-") as temp:
        artifact = Path(temp) / "artifact"
        if args.baseline_binary:
            report["v27_baseline"] = {
                "binary": str(args.baseline_binary.resolve()),
                "revision": args.baseline_revision,
                "fixture": "v27_vector_contiguous.ae",
                "timings": measure(args.baseline_binary, "v27_vector_contiguous.ae", args.runs, artifact),
            }

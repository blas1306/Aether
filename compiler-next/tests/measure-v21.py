#!/usr/bin/env python3
"""V21 warm-cache snapshots using the eight V17 core phase boundaries.
...
Optional baseline binary must be built separately from the unmodified V20 tree.
"""
import argparse
import json
...
FIXTURES = (
    "v20_remove_buffer.ae", "v21_vector_row.ae", "v21_vector_column.ae",
    "v21_vector_loop.ae", "v21_vector_owning.ae",
)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="aether-v21-timing-") as temp:
        artifact = Path(temp) / "artifact"
        if args.baseline_binary:
            report["v20_baseline"] = {
                "binary": str(args.baseline_binary.resolve()),
                "revision": args.baseline_revision,
                "fixture": "v20_remove_buffer.ae",
                "timings": measure(args.baseline_binary, "v20_remove_buffer.ae", args.runs, artifact),
            }

#!/usr/bin/env python3
"""OOP-OPT-1: compare true compiler O0/O2, physical events/sites and native cost.
...
def main():
    ...
    sources = sorted((workspace / 'tests/programs').glob('oop_v[12]_cost_*.ae'))
    sources += sorted((workspace / 'tests/programs').glob('oop_opt_1_*.ae'))
    rows = []
        ...
        for source in sources:
            ...
            for opt in ['O0', 'O2']:
                ...
                bodies = '\n'.join(re.findall(r'^define [^\n]*@__aether_[\s\S]*?^}',
                                               llvm, re.MULTILINE))
                sites = dict(retain=bodies.count('call void @aether_object_retain('),

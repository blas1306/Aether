#!/usr/bin/env python3
"""V22 warm-cache snapshots using the eight V17 core phase boundaries.
...
Optional baseline binary must be built separately from the unmodified V21 tree.
"""
import argparse
import json
...
FIXTURES = (
    "v21_vector_row.ae", "v22_transpose_row.ae", "v22_transpose_column.ae",
    "v22_transpose_owning.ae", "v22_transpose_double.ae",
)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="aether-v22-timing-") as temp:
        artifact = Path(temp) / "artifact"
        if args.baseline_binary:
            report["v21_baseline"] = {
                "binary": str(args.baseline_binary.resolve()),
                "revision": args.baseline_revision,
                "fixture": "v21_vector_row.ae",
                "timings": measure(args.baseline_binary, "v21_vector_row.ae", args.runs, artifact),
            }

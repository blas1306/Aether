#!/usr/bin/env python3
"""V30 warm-cache snapshots using the eight V17 core phase boundaries.
...
Optional baseline binary must be built separately from the unmodified V29 tree.
"""
import argparse
import json
...
FIXTURES = (
    ...
    "programs/v30_vector_add.ae",
    "programs/v30_vector_scale.ae",
    "programs/v30_matrix_add.ae",
    "programs/v30_matrix_transposed.ae",
)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="aether-v30-timing-") as temp:
        artifact = Path(temp) / "artifact"
        if args.baseline_binary:
            report["v29_baseline"] = {
                "binary": str(args.baseline_binary.resolve()),
                "revision": args.baseline_revision,
                "fixture": "programs/v29_add_int.ae",
                "timings": measure(args.baseline_binary, "programs/v29_add_int.ae", args.runs, artifact),
            }

#!/usr/bin/env python3
"""V25 warm-cache snapshots using the eight V17 core phase boundaries.
...
Optional baseline binary must be built separately from the unmodified V24 tree.
"""
import argparse
import json
...
FIXTURES = (
    "v24_matrix_view_normal.ae", "v25_vector_view_normal.ae",
    "v25_vector_view_mutable.ae", "v25_vector_view_transpose.ae",
    "v25_vector_view_projected.ae",
)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="aether-v25-timing-") as temp:
        artifact = Path(temp) / "artifact"
        if args.baseline_binary:
            report["v24_baseline"] = {
                "binary": str(args.baseline_binary.resolve()),
                "revision": args.baseline_revision,
                "fixture": "v24_matrix_view_normal.ae",
                "timings": measure(args.baseline_binary, "v24_matrix_view_normal.ae", args.runs, artifact),
            }

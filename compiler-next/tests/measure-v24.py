#!/usr/bin/env python3
"""V24 warm-cache snapshots using the eight V17 core phase boundaries.
...
Optional baseline binary must be built separately from the unmodified V23 tree.
"""
import argparse
import json
...
FIXTURES = (
    "v23_matrix_rectangular.ae", "v24_matrix_view_normal.ae",
    "v24_matrix_view_transpose.ae", "v24_matrix_view_transpose_mut.ae",
    "v24_matrix_view_projected.ae",
)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="aether-v24-timing-") as temp:
        artifact = Path(temp) / "artifact"
        if args.baseline_binary:
            report["v23_baseline"] = {
                "binary": str(args.baseline_binary.resolve()),
                "revision": args.baseline_revision,
                "fixture": "v23_matrix_rectangular.ae",
                "timings": measure(args.baseline_binary, "v23_matrix_rectangular.ae", args.runs, artifact),
            }

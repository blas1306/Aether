#!/usr/bin/env python3
"""V27 warm-cache snapshots using the eight V17 core phase boundaries.
...
Optional baseline binary must be built separately from the unmodified V26 tree.
"""
import argparse
import json
...
FIXTURES = (
    "v26_matrix_axis_column.ae", "v27_vector_contiguous.ae",
    "v27_vector_strided.ae", "v27_matrix_contiguous.ae",
    "v27_matrix_transposed.ae",
)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="aether-v27-timing-") as temp:
        artifact = Path(temp) / "artifact"
        if args.baseline_binary:
            report["v26_baseline"] = {
                "binary": str(args.baseline_binary.resolve()),
                "revision": args.baseline_revision,
                "fixture": "v26_matrix_axis_column.ae",
                "timings": measure(args.baseline_binary, "v26_matrix_axis_column.ae", args.runs, artifact),
            }

#!/usr/bin/env python3
"""V29 warm-cache snapshots using the eight V17 core phase boundaries.
...
Optional baseline binary must be built separately from the unmodified V28 tree.
"""
import argparse
import json
...
FIXTURES = (
    "programs/v28_vector_contiguous.ae",
    "programs/v29_add_int.ae",
    "programs/v29_affine_int.ae",
    "programs/v29_affine_float.ae",
    "modules/v29_behavior/main.ae",
)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="aether-v29-timing-") as temp:
        artifact = Path(temp) / "artifact"
        if args.baseline_binary:
            report["v28_baseline"] = {
                "binary": str(args.baseline_binary.resolve()),
                "revision": args.baseline_revision,
                "fixture": "programs/v28_vector_contiguous.ae",
                "timings": measure(args.baseline_binary, "programs/v28_vector_contiguous.ae", args.runs, artifact),
            }

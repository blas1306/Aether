#!/usr/bin/env python3
"""V18 warm-cache snapshots using the eight V17 core phase boundaries.
...
Optional baseline binary must be built separately from the unmodified V17 tree.
"""
import argparse
import json
...
FIXTURES = (
    "v17_storable.ae", "v18_pop_int.ae", "v18_pop_buffer.ae",
    "v18_pop_nested.ae", "v18_pop_reuse.ae",
)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="aether-v18-timing-") as temp:
        artifact = Path(temp) / "artifact"
        if args.baseline_binary:
            report["v17_baseline"] = {
                "binary": str(args.baseline_binary.resolve()),
                "revision": args.baseline_revision,
                "fixture": "v17_storable.ae",
                "timings": measure(args.baseline_binary, "v17_storable.ae", args.runs, artifact),
            }

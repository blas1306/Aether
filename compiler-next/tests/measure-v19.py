#!/usr/bin/env python3
"""V19 warm-cache snapshots using the eight V17 core phase boundaries.
...
Optional baseline binary must be built separately from the unmodified V18 tree.
"""
import argparse
import json
...
FIXTURES = (
    "v18_pop_buffer.ae", "v19_swap_remove_int.ae", "v19_swap_remove_buffer.ae",
    "v19_swap_remove_nested.ae", "v19_swap_remove_reuse.ae",
)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="aether-v19-timing-") as temp:
        artifact = Path(temp) / "artifact"
        if args.baseline_binary:
            report["v18_baseline"] = {
                "binary": str(args.baseline_binary.resolve()),
                "revision": args.baseline_revision,
                "fixture": "v18_pop_buffer.ae",
                "timings": measure(args.baseline_binary, "v18_pop_buffer.ae", args.runs, artifact),
            }

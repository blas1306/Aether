...
def test_readiness_artifact_is_byte_deterministic() -> None:
...
def test_inventory_covers_every_current_instruction_dataclass() -> None:
    ...
    assert [row["name"] for row in report["inventories"]["initial_ir_instruction_types"]] == [
        item.__name__ for item in module._concrete_subclasses(module.ir.IRInstruction)
    ]
    assert [row["name"] for row in report["inventories"]["ssa_instruction_types"]] == [
        item.__name__ for item in module._concrete_subclasses(module.ssa.SSAInstruction)
    ]

...
| IRV-024 | Non-void exits return values | From `entry`, every reachable function exit in a non-void function is a return carrying a value. Cycles are non-exiting paths and do not create a missing-return obligation; unreachable regions are outside this graph property. Non-entry block labels are identifiers only, so arbitrary bijective renaming preserves the result. Independent local instruction checks, including IRV-025, still apply to retained unreachable blocks. | `_verify_all_non_void_paths_return` | Returns |

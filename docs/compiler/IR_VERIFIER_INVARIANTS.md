...
| IRV-024 | Non-void paths return values | From `entry`, every exiting path of a non-void function reaches a return carrying a value. Back-edges encountered at blocks named with `cond` or `for.cond` prefixes are treated as non-exiting loop cycles; other encountered cycles do not prove a return. | `_verify_all_non_void_paths_return`, `_block_returns` (lines 2059–2108) | Returns |

...
| --- | --- | --- |
| Exceptions | **Implemented internally; promotion blocked for 1.x** | Frozen semantics, explicit cleanup, verified SSA, private event-out lowering, runtime and boundary containment exist. Hotfixes A–D closed ERQ-001 through ERQ-005 without promotion; integrated public release evidence and packaging remain before the stable gate can change. `finally`, hierarchies, resumable exceptions, public FFI and general unwinding are not implied. Result/status types remain preferred for expected numerical failures. |

...
| Aritmética `+ - * / % ^` | Implemented | Implemented | Tipos reales/complex/agregados | int/double y agregados seleccionados; casts mixtos explícitos | Igual; folds checked | int/double, `frem`, helper pow i32 y libm | Igual | Highlight | paridad numérica E2E | Partial por float/complex/agregados |
...
| Cast explícito | Implemented como llamada de tipo | Amplio en frontend | int/float/double/complex/string/boolean con límites | int↔double e identidad | mismo subset | i32↔double; identidad eliminada/no-op | Implemented para ese par | Completion | frontend + par nativo | Partial |
| Conversión implícita | Implemented | widening de frontend | Implemented | `int→double` target-typed y en operaciones mixtas | casts preservados | consume solo IR homogéneo | subset int/double | Diagnósticos | E2E contextual | Partial por otras conversiones |
...
| Native Interface ABI | Implemented | conformidad nominal | construcción class→interface | `InterfaceType`, `interface_construct`, witness metadata | valores/phis y optimizadores | `{ptr carrier, ptr witness}` + tablas constantes | transporte/ARC carrier-only | completion/symbols | ABI/DTO/verifier/LLVM | Implemented 5.4A |
| Interface dispatch class-carrier | Implemented | firma/conformidad | dispatch class | `interface_call`, slot/firma borrada | preserva efectos/ARC/panic | thunks + witness load + indirect call | receiver borrowed; mutación alias-visible | completion/symbols | Python/Rust/native O0/O1/O2 | Implemented 5.4B |
| Native Struct-backed Interfaces | Implemented | conformidad nominal | conversión y dispatch struct | box layout + `copy_owned`/`drop_owned` | boxing y adapters no eliminables | caja privada + unboxing thunk | copia profunda de caja, teardown recursivo | diagnostics | DTO/Python/Rust/native O0/O1/O2 | Implemented 5.4C |

...
The runtime creates the class instance first, then executes the constructor body with that instance as borrowed `this`, and finally returns the initialized instance. Every field must be assigned on every successful path before it is read or the constructor returns; no field category receives an implicit default, including nullable fields. The current grammar has no field-declaration initializer expression, so initialization occurs in the explicit constructor body or through the automatic positional constructor. A loop does not prove initialization because it may execute zero times, and the incomplete `this` cannot be passed or otherwise exposed as a value. A constructor is an instance context: it may read and write fields, access private members of its own class, and use its parameters. `return;` may end a constructor only after all fields are initialized; `return` with a value is invalid. Calling general class methods from native constructors remains outside the Phase 5.3B subset.
